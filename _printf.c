...
/**
 * _process_format - Processes the format string and handles specifiers.
...
void _process_format(const char *format, int *printed_chars, va_list args)
{
	...
	for (ptr = format; *ptr != '\0'; ++ptr)
	{
		if (*ptr == '%')
		{
			...
			{
				char *specifier_output = process_format_specifier(&ptr, args, printed_chars);
...
/**
 * process_format_specifier - Processes a format specifier and updates the output buffer.
 * @ptr: Pointer to the current position in the format string.
 * @args: Variable arguments corresponding to the format specifier.
 * @printed_chars: Pointer to the count of printed characters.
 * 
 * Description: This function processes a valid format specifier and updates the output buffer
 * with the formatted output. It handles different specifier cases ('c', 's', 'd', 'i').
 * After processing the specifier, the function increments the pointer to the format string,
 * updates the printed character count, and modifies the output buffer accordingly.
 */
 ...
 * 
 * Description: Appends the input string to the buffer, reallocating memory if needed.
 */
void append_string(char **buffer, const char *str, int *output_len)
{
	if (*buffer == NULL)
	...
	else
	{
		...
		*buffer = (char *)realloc(*buffer, (*output_len + str_len + 1) * sizeof(char));
